...
> **Reference-only install** — Only `SKILL.md` + `references/` are vendored in this project. The upstream `scripts/`, `data/`, and Node/Python tooling (incl. slide generation) are **not** included — use the `references/` docs below as design-token guidance. Source: github.com/nextlevelbuilder/ui-ux-pro-max-skill (MIT).
