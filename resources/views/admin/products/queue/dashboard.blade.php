<x-admin-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            {{-- Notifications are configured centrally for all products. --}}
            <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-6 py-4 shadow-sm">
                <div>
                    <h3 class="text-sm font-semibold text-slate-900">{{ __('app.admin.notifications.heading') }}</h3>
                    <p class="mt-0.5 text-xs text-slate-500">{{ __('app.admin.products.queue.notify_moved_hint') }}</p>
                </div>
                <a href="{{ route('admin.notifications.edit') }}"
                   class="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    {{ __('app.admin.notifications.heading') }} →
                </a>
            </div>
        </div>
    </div>
