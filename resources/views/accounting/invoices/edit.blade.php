<x-accounting-layout>
    <x-slot name="header">
        <div>
            <a href="{{ route('accounting.invoices.show', $invoice) }}" class="text-xs text-slate-500 hover:text-slate-700">← {{ $invoice->no }}</a>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('app.accounting.invoice_edit') }}</h2>
        </div>
    </x-slot>

    @php
        // Best-effort match: pick the WHT code whose rate matches the stored ratio
        $whtRateGuess = ((float) $invoice->subtotal) > 0
            ? round(((float) $invoice->wht_amount) / ((float) $invoice->subtotal) * 100, 2)
            : 0;
        $currentWhtCodeId = '';
        foreach ($whtCodes as $code) {
            if (round((float) $code->rate, 2) === $whtRateGuess) {
                $currentWhtCodeId = (string) $code->id;
                break;
            }
        }
    @endphp
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('accounting.invoices.update', $invoice) }}" x-data="invoiceForm()" @submit="submitting = true" class="space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="partner_id" name="partner_id" required class="mt-1.5 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                                @foreach ($partners as $partner)
                                    <option value="{{ $partner->id }}" @selected(old('partner_id', $invoice->partner_id) == $partner->id)>{{ $partner->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input id="issue_date" name="issue_date" type="date" required value="{{ old('issue_date', $invoice->issue_date?->toDateString()) }}"
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('accounting.invoices.show', $invoice) }}" class="text-sm text-slate-600 hover:text-slate-900">{{ __('app.common.cancel') }}</a>
    ...
    <script>
        function invoiceForm() {
            return {
                submitting: false,
                lines: @js($invoice->lines->map(fn ($l) => [
