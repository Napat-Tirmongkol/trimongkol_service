    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('app.accounting.purchases_by_partner') }}</h2>
                <p class="mt-0.5 text-sm text-slate-500">{{ __('app.accounting.purchases_by_partner_sub') }}</p>
            </div>
            <form method="GET" action="{{ route('accounting.reports.purchases-by-partner') }}" class="flex flex-wrap items-end gap-2 sm:shrink-0">
                <div>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-6xl space-y-4 px-4 sm:px-6 lg:px-8">
            @include('accounting.reports._by-partner-table', [
                'report' => $report,
                'partnerLabel' => __('app.accounting.vendor'),
                'totalLabel' => __('app.accounting.purchases_total'),
            ])
        </div>
    </div>
</x-app-layout>
