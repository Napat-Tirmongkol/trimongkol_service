    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('app.accounting.aged_ap') }}</h2>
                <p class="mt-0.5 text-sm text-slate-500">{{ __('app.accounting.aged_ap_sub') }}</p>
            </div>
            <form method="GET" action="{{ route('accounting.reports.aged-ap') }}" class="flex flex-wrap items-end gap-2 sm:shrink-0">
                <div>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-6xl space-y-4 px-4 sm:px-6 lg:px-8">
            ...
            @include('accounting.reports._aging-table', [
                'report' => $report,
                'totalLabel' => __('app.accounting.aged_ap_total'),
            ])
        </div>
    </div>
</x-accounting-layout>
