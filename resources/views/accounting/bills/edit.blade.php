<x-app-layout>
    <x-slot name="header">
        <div>
            <a href="{{ route('accounting.bills.show', $bill) }}" class="text-xs text-slate-500 hover:text-slate-700">← {{ $bill->no }}</a>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('app.accounting.bill_edit') }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('accounting.bills.update', $bill) }}" x-data="billForm()" @submit="submitting = true" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <select id="partner_id" name="partner_id" required class="mt-1.5 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                                @foreach ($vendors as $vendor)
                                    <option value="{{ $vendor->id }}" @selected(old('partner_id', $bill->partner_id) == $vendor->id)>{{ $vendor->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input id="issue_date" name="issue_date" type="date" required value="{{ old('issue_date', $bill->issue_date?->toDateString()) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input id="bill_ref" name="bill_ref" type="text" maxlength="120" value="{{ old('bill_ref', $bill->bill_ref) }}"
                                   class="mt-1.5 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('accounting.bills.show', $bill) }}" class="text-sm text-slate-600 hover:text-slate-900">{{ __('app.common.cancel') }}</a>
    ...
    <script>
        function billForm() {
            return {
                ...
                addLine() { this.lines.push({ description: '', quantity: 1, unit_price: '', account_id: '', tax_code_id: '' }); },
