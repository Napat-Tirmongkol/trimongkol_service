...
ส่วนงวดทั้งหมดเป็นแค่ progress bar อ่านอย่างเดียว ตอนนี้แก้ได้เองครบ:

- **เพิ่มงวด** — กดปุ่ม "เพิ่มงวด" ใส่เดือนครบกำหนด + ยอดเป้า เองได้ (สร้างจาก 0 ก็ได้ถ้ายังไม่มีงวด)
- **แก้งวด** — แก้ยอดเป้า + เดือนครบกำหนด ของแต่ละงวดในรายการ "จัดการทั้ง N งวด"
- **ลบงวด** — ลบงวดที่ไม่ต้องการ (พร้อมประวัติจ่ายของงวดนั้น)
- **บันทึก/ลบการจ่ายของงวดไหนก็ได้** — ไม่จำกัดแค่งวดปัจจุบันแล้ว
- backend: `updateDebtPayment` รับ `due_month` เพิ่ม + คำนวณ is_paid ใหม่เมื่อแก้เป้า (เฉพาะงวดที่มียอดจ่ายจริง — ไม่กระทบหนี้ตารางปกติ)
- สร้าง กยศ ใหม่จาก 0 ได้: เพิ่มหนี้ผ่อนตามตารางที่ชื่อมีคำว่า "กยศ" แล้วใส่งวดเอง

---

## 🔧 แผนหนี้ /portfolio/debts — กยศ กลับมาแสดงแบบ "เก็บรายเดือน"

เดิม กยศ ถูกเก็บเป็นเป้าหมายรายปีก้อนเดียว (ครบกำหนด 5 ก.ค.) แล้ว `DebtOverviewController`
วาดยอดทั้งก้อนลงเดือน ก.ค. เดือนเดียว — ทำให้มุมมอง "เก็บรายเดือนเพื่อจ่ายให้ครบเป้า"
ที่ตั้งใจไว้หายไป (เห็นเป็นก้อนใหญ่ปีละครั้งแทน)

- ตอนนี้ controller **เฉลี่ยยอดคงเหลือของแต่ละเป้ารายปีลงช่วงเก็บเงิน 10 เดือน (ก.ย.–มิ.ย.)** ก่อนถึงกำหนด ก.ค.
- ยอดต่อเดือน = คงเหลือ ÷ จำนวนเดือนที่ยังเก็บได้ → ปรับตามที่จ่ายจริง (`paid_amount` จาก payment logs) อัตโนมัติ
- ถ้าช่วงเก็บผ่านไปแล้วแต่ยังค้าง จะตกไปแสดงเป็นก้อนที่เดือน ก.ค. ตามเดิม
- **ไม่แตะโครงสร้างข้อมูล** — target รายปี + payment logs (จ่ายจริงไม่สม่ำเสมอ) ยังอยู่ครบ แก้แค่วิธีแสดงผล ไม่ต้อง migrate
- **เพิ่มยอดรวมในหน้างบ** — การ์ด "หนี้สิน / ผ่อนของ" มีบรรทัด "รวมผ่อนต่อเดือน (ใช้จริง / งบ)" ท้ายการ์ด แบบเดียวกับ "รวมค่าใช้จ่ายคงที่" (ผ่อน + ชำระหนี้ ต่อเดือน · อัปเดตสดเมื่อติ๊กว่าจ่ายแล้ว)
