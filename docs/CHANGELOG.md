...
## 🧾 อ่านใบเสร็จ e-receipt (PDF) → ลงรายรับรายจ่ายอัตโนมัติ (เฟส 1)

อัปโหลด e-receipt 7-Eleven (PDF) ในหน้า `/portfolio/ledger` แล้วระบบดึง วันที่/ยอด/ร้าน ให้อัตโนมัติ

- **อ่านในเบราว์เซอร์ด้วย pdf.js (โหลดผ่าน CDN)** — ไม่เพิ่ม composer/npm dependency, ไม่ส่งไฟล์ขึ้นเซิร์ฟเวอร์
- e-receipt เป็น PDF ที่มี text layer → **ไม่ต้องใช้ OCR** วันที่/ยอดเงินแม่นเต็ม (แปลง พ.ศ. → ค.ศ. ให้)
- ดึงค่าแล้ว **เติมลงฟอร์ม "เพิ่มรายการ" เดิม** (วันที่ · ยอด · ร้าน · เลขใบกำกับในหมายเหตุ) ให้ตรวจก่อนกดบันทึก
- ใช้ endpoint เดิม `portfolio.ledger.store` — ไม่แตะ backend, ไม่เพิ่มตาราง · คีย์ `app.portfolio.ledger.receipt_*` (TH/EN)
- เฟสถัดไป (ยังไม่ทำ): ดึงจาก Gmail อัตโนมัติ (ต้องขอสิทธิ์ + dependency เพิ่ม)
