...
- **`docs/POS.md`** — ขอบเขต v1 · การตัดสินใจหลัก 8 ข้อ (ตาราง `pos_*` แยกจากบัญชี, auth แบบ queue, เงินสตางค์ integer, ราคารวม VAT/ใบกำกับภาษีอย่างย่อ, บิล immutable) · data model 7 ตาราง · services · routes+สิทธิ์ · UI จอขาย tablet-first · แพ็กเกจ Free/฿299/฿699
- **reuse ของเดิมล้วน** — workspace, ProductGate, pattern แพ็กเกจ queue/accounting, `PromptPay`+`SlipVerifier`, qrcode CDN — **ไม่มี dependency ใหม่**
- แผนสร้าง 5 เฟส ตัดจบใช้ได้จริงทุกเฟส: คลังสินค้า → จอขาย+เงินสด+ใบเสร็จ (dogfood ได้) → กะ+PromptPay+รายงาน → แพ็กเกจ+เปิดขาย → เชื่อมระบบบัญชี
