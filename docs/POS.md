...
- ส่งครัวเป็นรอบ ๆ + **ใบส่งครัวแยกตามสเตชั่น** — ร้านที่ครัวแบ่งหลายจุด (เช่น ปิ้งย่าง /
  ส้มตำ / ครัว / คาเฟ่เครื่องดื่ม) แต่ละสเตชั่นได้ใบเฉพาะจานของตัวเอง พิมพ์ที่เครื่องพิมพ์
  ประจำจุดได้
...
| Kitchen Display (จอครัว) | v1 ใช้ใบส่งครัวพิมพ์ก่อน — หน้า print-station กรองตามสเตชั่นทำหน้าที่เป็นรายการงานแบบหยาบไปพลาง · KDS เต็มรูป (จอสัมผัส กดเสร็จรายจาน) ค่อยเพิ่มบนโครงสเตชั่นเดิมได้เลย |
...
**D12 — สเตชั่นครัวเป็น first-class: ใบครัวแยกตามสเตชั่นต่อรอบ**
ร้านกำหนดสเตชั่นเองได้ (เช่น ปิ้งย่าง / ส้มตำ / ครัว / คาเฟ่เครื่องดื่ม) — mapping 2 ชั้น:
**หมวดหมู่กำหนดสเตชั่นเริ่มต้น เมนูรายตัว override ได้** (ของส่วนใหญ่ตั้งครั้งเดียวที่หมวด)
กด "ส่งครัว" 1 ครั้ง = แตกใบครัวเป็น **1 ใบต่อสเตชั่นที่มีจานในรอบนั้น** แต่ละใบเป็น
print job แยกพร้อม `station_id` → จุดพิมพ์แต่ละจุดเลือกรับเฉพาะสเตชั่นตัวเอง
(หรือเครื่องเดียวรับทุกใบถ้าร้านมีปริ้นเตอร์ตัวเดียว) · ร้านที่ไม่ตั้งสเตชั่นเลย = ใบครัวรวมใบเดียว
ตามเดิม · จานที่ยังไม่ระบุสเตชั่นไปรวมในใบ "ยังไม่ระบุสเตชั่น" — ไม่มีจานตกหล่น
...
workspaces 1─n pos_stations ←─ station_id บน categories (default) + products (override)
...
### `pos_stations` (โหมดร้านอาหาร — สเตชั่นครัว)

`workspace_id` · `name` ("ปิ้งย่าง", "ส้มตำ", "ครัว", "คาเฟ่เครื่องดื่ม") · `sort_order` ·
`is_active` · unique `(workspace_id, name)` — ไม่ตั้งเลย = ใบครัวรวมใบเดียว

### `pos_categories`

`workspace_id` · `name` · `color` (key จาก palette คงที่ — ดู §6 เรื่อง dynamic Tailwind) ·
`station_id` (FK nullable — **สเตชั่นเริ่มต้นของทั้งหมวด** เช่น หมวด "เครื่องดื่ม" →
คาเฟ่) · `sort_order` · timestamps · unique `(workspace_id, name)`
...
| station_id | FK nullable | override สเตชั่นรายเมนู — null = ใช้ของหมวด (เช่น "ข้าวเหนียว" อยู่หมวดส้มตำแต่ออกที่ครัว) |
...
| station_id | FK nullable — ส่งครัว 1 รอบแตกเป็น 1 job/สเตชั่น (null = ใบรวม/ใบเสร็จ) |
...
| `Pos\KitchenTicket` | "ส่งครัว": ตีตรา round_no + sent_at ให้รายการ pending → หา**สเตชั่นผลลัพธ์**ของแต่ละจาน (product.station ?? category.station) → **แตกเป็นใบครัว 1 ใบ/สเตชั่น** ที่มีจานในรอบนั้น แล้ว enqueue ลง `PrintQueue` (ร้านไม่มีสเตชั่น = ใบรวมใบเดียว) |
...
| `Pos\Reports` | ยอดขายช่วงวันที่ แยก วิธีจ่าย/สินค้า/หมวด/**สเตชั่น**/พนักงาน/ประเภทออเดอร์ + ยอด SC รวม + CSV (UTF-8 BOM แบบ `TaxReporting`) — CSV เป็น flag ของแพ็กเกจ · ยอดต่อสเตชั่นบอกได้เลยว่าปิ้งย่างหรือส้มตำขายดีกว่ากัน |
...
CRUD /pos/tables, /pos/stations, /pos/modifier-groups            [owner/admin]
...
```
มือถือพนักงาน (จอสั่ง โต๊ะ 3)            จุดพิมพ์ "สเตชั่นปิ้งย่าง"
┌──────────────────┐                  ┌──────────────────────────────┐
│ ← โต๊ะ 3 · 2 ที่นั่ง│                  │ /pos/print-station            │
│ [ค้นหา ________] │                  │ รับงาน: [ปิ้งย่าง ▾]          │
│ [ตำ][ย่าง][น้ำ]  │   ส่งครัว         │ ● รอพิมพ์ 1 งาน               │
│ ┌──────┐┌──────┐ │   ───────→      │ 14:32 ใบครัว·ปิ้งย่าง โต๊ะ3 ร.2│
│ │คอหมู ││ตำไทย │ │  แตกใบตาม       │   → พิมพ์อัตโนมัติ ✓          │
│ │ 120.-││ 60.- │ │  สเตชั่น:        │ 14:21 ใบครัว·ปิ้งย่าง โต๊ะ1 ✓  │
│ └──────┘└──────┘ │  ปิ้งย่าง+ส้มตำ   └──────────────────────────────┘
│──────────────────│  +คาเฟ่           (จุดส้มตำ/ครัว/คาเฟ่ ก็เปิดหน้า
│ 🛒 3 รายการ 250.- │                   เดียวกัน เลือกสเตชั่นของตัวเอง)
│ [    ส่งครัว    ] │
└──────────────────┘
```

ตัวอย่างจริง — ครัวแบ่ง 4 สเตชั่น: **ปิ้งย่าง / ส้มตำ / ครัว / คาเฟ่เครื่องดื่ม**
โต๊ะ 3 สั่ง คอหมูย่าง + ตำไทย + ชาเย็น → กดส่งครัวครั้งเดียว ระบบแตกเป็น 3 ใบ:
ใบปิ้งย่าง (คอหมู) · ใบส้มตำ (ตำไทย) · ใบคาเฟ่ (ชาเย็น) — แต่ละใบโผล่ที่จุดพิมพ์ของ
สเตชั่นนั้น ไม่ต้องมีคนวิ่งฉีกบิลแจก

- **จุดพิมพ์** = เครื่องที่ต่อเครื่องพิมพ์ (แท็บเล็ต/คอม/มือถือเก่าเสียบชาร์จไว้ก็ได้)
  เปิดหน้า `/pos/print-station` ค้างไว้ แล้ว**เลือกว่ารับสเตชั่นไหน** (เลือกหลายอันหรือ
  "ทุกสเตชั่น + ใบเสร็จ" ได้ — จำค่าไว้ในเครื่องนั้น) → poll ทุก ~5 วิ เจองานของตัวเอง →
  render ใบ → `window.print()` → ตีตรา printed
- ปริ้นเตอร์ตัวเดียวทั้งร้านก็ใช้ได้: เครื่องนั้นรับ "ทุกสเตชั่น" — ใบยังแตกตามสเตชั่นอยู่
  (หัวใบบอกสเตชั่นตัวใหญ่ ๆ) แค่ออกที่เครื่องเดียว ฉีกแจกตามจุดได้ทันที
...
- จอสั่งโชว์ชื่อสเตชั่นเล็ก ๆ ใต้แต่ละจาน — พนักงานเห็นทันทีว่าจานนี้จะวิ่งไปจุดไหน
...
| สเตชั่นครัว/แยกใบพิมพ์ (`max_stations`) | ใบรวมใบเดียว | ✓ ไม่จำกัด | ✓ ไม่จำกัด |
| เงินสด + PromptPay QR | ✓ | ✓ | ✓ |
| กะ + รายงานขาย | ✓ | ✓ | ✓ |
...
### Phase R1 — ชั้นร้านอาหาร: โต๊ะ + ออเดอร์ + มือถือพนักงาน + สเตชั่นครัว (~6–7 วัน)
- Migrations: `pos_tables`, `pos_stations`, `pos_orders`, `pos_order_items`,
  `pos_modifier_groups`, `pos_modifiers`, `pos_product_modifier_group`, `pos_print_jobs`
  (+ คอลัมน์ order/SC บน `pos_sales`, `station_id` บน categories/products)
- ผังโต๊ะ + เปิดออเดอร์/กันซ้อนโต๊ะ + จอสั่ง **responsive ถึงมือถือพนักงาน** (modifier popup +
  โน้ต + `added_by`) + ส่งครัวเป็นรอบ + **ใบครัวแตกตามสเตชั่น** + จุดพิมพ์เลือกสเตชั่นได้ +
  ย้ายโต๊ะ + ยกเลิกจาน/ออเดอร์ตามกติกา + service charge + เก็บเงินผ่าน `Checkout` เดิม +
  CRUD โต๊ะ/สเตชั่น/ตัวเลือก
- **Tests:** OrderFlowTest (เปิด→สั่ง→ส่งครัว→สั่งเพิ่ม→เก็บเงิน→โต๊ะว่าง), กันออเดอร์ซ้อนโต๊ะ,
  ...
  (สูตร SC→VAT), MoveTableTest, **StationRoutingTest** (จานกระจายถูกใบ — override รายเมนู
  ชนะหมวด, ไม่ระบุ→ใบ "ยังไม่ระบุ", ไม่มีสเตชั่น→ใบรวม), PrintQueueTest (enqueue แยกใบ
  ตามสเตชั่น, จุดพิมพ์กรองสเตชั่น, claim atomic กันพิมพ์ซ้ำ), สองคนสั่งโต๊ะเดียวกันพร้อมกัน
  (added_by ถูกคน รายการไม่หาย)
- **DoD:** **ร้านนำร่อง (ครัว 4 สเตชั่น: ปิ้งย่าง/ส้มตำ/ครัว/คาเฟ่เครื่องดื่ม) รับลูกค้าจริง
  ได้ทั้งวัน — พนักงานเสิร์ฟถือมือถือตัวเองรับออเดอร์ที่โต๊ะ กดส่งครัวครั้งเดียวแล้วใบครัว
  ออกแยกครบทุกสเตชั่นที่เกี่ยว**
...
- ตามด้วย (เรียงตามเสียงลูกค้า): แยกบิล/รวมโต๊ะ · จอครัวต่อสเตชั่น (KDS — ต่อยอดจาก
  print-station + โครงสเตชั่นที่มีแล้ว กดเสร็จรายจาน) · สมาชิก/แต้ม · split payment UI ·
  PIN พนักงาน · พิมพ์ฉลากบาร์โค้ด
...
| จุดพิมพ์ถูกปิด/ลืมเปิด (สเตชั่นใดสเตชั่นหนึ่ง) | งานค้างอยู่ในคิวรายสเตชั่น (ไม่หาย) + ทุกเครื่องเห็น badge "รอพิมพ์ N" กดพิมพ์แทนได้ + พิมพ์ซ้ำได้เสมอ |
| งานพิมพ์ถูกพิมพ์ซ้ำสองเครื่อง | claim แบบ atomic (`UPDATE … WHERE status='pending'`) — เครื่องเดียวได้งาน |
| เมนูลืมระบุสเตชั่น → จานหาย | ไม่หาย — รวมเข้าใบ "ยังไม่ระบุสเตชั่น" เสมอ + หน้า CRUD เมนูโชว์คอลัมน์สเตชั่นให้เห็นช่องว่าง |
